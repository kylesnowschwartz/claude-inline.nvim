...
function M.sha1(data)
  ...
  for chunk_start = 1, #msg, 64 do
    ...
    for i = 0, 15 do
      ...
      w[i] = bor(bor(bor(lshift(msg:byte(offset), 24), lshift(msg:byte(offset + 1), 16)), lshift(msg:byte(offset + 2), 8)), msg:byte(offset + 3))
    end
  ...
  for _, h in ipairs { h0, h1, h2, h3, h4 } do
    result = result .. string.char(band(rshift(h, 24), 0xFF), band(rshift(h, 16), 0xFF), band(rshift(h, 8), 0xFF), band(h, 0xFF))
  end
...
---@param response string The HTTP response string
---@return table headers Table of header name -> value pairs
function M.parse_http_headers(response)
  local headers = {}
  local lines = {}
