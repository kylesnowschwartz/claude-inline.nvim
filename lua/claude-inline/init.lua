  ...
  if msg_type == 'stream_event' then
    ...
    if event.type == 'content_block_start' then
      ...
      if block.type == 'thinking' then
        ...
        ui.hide_loading()
        ui.show_thinking()
        M._state.content_blocks[index] = { type = 'thinking' }
      elseif block.type == 'tool_use' then
        -- Starting a tool use block
        ui.hide_loading()
        -- Collapse thinking if it was active
        if M._state.thinking_text ~= '' then
    ...
    if content then
      for _, block in ipairs(content) do
        if block.type == 'tool_result' then
          ui.show_tool_result(block.tool_use_id, block.content or '', block.is_error)
          ui.collapse_tool_result(block.tool_use_id)
        end
      end
    end
