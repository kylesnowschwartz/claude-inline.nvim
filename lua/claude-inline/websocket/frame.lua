...
function M.create_frame(opcode, payload, fin, masked)
  ...
  masked = masked == true -- Default to false for server
  ...
  -- Add mask if needed (clients MUST mask)
  local mask = nil
  if masked then
...
---Create a text frame (unmasked for server use)
...
---Create a close frame (unmasked for server use)
...
---Create a ping frame (unmasked for server use)
...
---Create a pong frame (unmasked for server use)
