...
function M.log_message(msg)
  ...
  elseif msg_type == 'assistant' then
    ...
    if type(content) ~= 'table' then
      M.log('msg', string.format('assistant: content is %s, not table', type(content)))
      return
    end
    local types = {}
  ...
  elseif msg_type == 'user' then
    -- Log user messages (which contain tool_result blocks or direct string output)
    local content = msg.message and msg.message.content
    if type(content) == 'string' then
      -- Direct string content (e.g., from /context command)
      local preview = content:sub(1, 80):gsub('\n', '\\n')
      M.log('msg', string.format('user: string content, preview="%s"', preview))
      return
    elseif type(content) ~= 'table' then
      M.log('msg', string.format('user: content is %s, not table', type(content)))
      return
    end
    local types = {}
    for _, block in ipairs(content) do
    ...
    -- Log tool_use_result metadata if present
    local meta = msg.tool_use_result
    local meta_str = ''
    if meta then
      local parts = {}
      if meta.durationMs then
        table.insert(parts, string.format('duration=%dms', meta.durationMs))
      end
      if meta.numFiles then
        table.insert(parts, string.format('files=%d', meta.numFiles))
      end
      if meta.exitCode then
        table.insert(parts, string.format('exit=%d', meta.exitCode))
      end
      if meta.truncated then
        table.insert(parts, 'truncated')
      end
      if #parts > 0 then
        meta_str = ' meta={' .. table.concat(parts, ',') .. '}'
      end
    end
    M.log('msg', string.format('user: content_types=[%s]%s', table.concat(types, ', '), meta_str))
