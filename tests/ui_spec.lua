...
local function test_show_tool_use()
  ...
  assert_no_error(function()
    ui.show_tool_use('tool_123', 'read_file', { path = 'test.lua' })
  ...
  assert_eq(ui._state.content_blocks['tool_123'].name, 'read_file', 'tool name should be stored')

  -- Verify buffer content
  local lines = vim.api.nvim_buf_get_lines(ui._state.sidebar_buf, 0, -1, false)
  local found_tool_header = false
  for _, line in ipairs(lines) do
    if line:match '> %[tool: read_file%]' then
      found_tool_header = true
      break
    end
  end
  assert_true(found_tool_header, 'should find tool header in buffer')
end

-- Test: update_tool_input accumulates JSON and updates display
local function test_update_tool_input()
  setup()
  ui.show_sidebar()
  ui.append_message('assistant', '')
  ui.show_tool_use('tool_456', 'bash', {})

  assert_no_error(function()
    ui.update_tool_input('tool_456', '{"command":')
    ui.update_tool_input('tool_456', '"ls -la"}')
  end, 'update_tool_input should not throw')

  -- Verify JSON accumulation
  assert_eq(ui._state.content_blocks['tool_456'].input_json, '{"command":"ls -la"}', 'input_json should accumulate')
end

-- Test: complete_tool finalizes block state
local function test_complete_tool()
  setup()
  ui.show_sidebar()
  ui.append_message('assistant', '')
  ui.show_tool_use('tool_789', 'write_file', { path = 'out.txt' })

  assert_no_error(function()
    ui.complete_tool('tool_789', 'success')
  end, 'complete_tool should not throw')

  assert_eq(ui._state.content_blocks['tool_789'].state, 'success', 'block state should be success')

  -- Verify buffer shows "done" status
  local lines = vim.api.nvim_buf_get_lines(ui._state.sidebar_buf, 0, -1, false)
  local found_done = false
  for _, line in ipairs(lines) do
    if line:match '> %[tool: write_file%] done' then
      found_done = true
      break
    end
  end
  assert_true(found_done, "should find 'done' status in buffer")
end

-- Test: show_tool_result creates result block
local function test_show_tool_result()
  setup()
  ui.show_sidebar()
  ui.append_message('assistant', '')
  ui.show_tool_use('tool_result_test', 'read_file', { path = 'test.lua' })
  ui.complete_tool 'tool_result_test'

  assert_no_error(function()
    ui.show_tool_result('tool_result_test', 'file contents here\nline 2\nline 3', false)
  end, 'show_tool_result should not throw')

  -- Verify result block created
  local result_id = 'result_tool_result_test'
  assert_true(ui._state.content_blocks[result_id] ~= nil, 'result block should be tracked')
  assert_eq(ui._state.content_blocks[result_id].type, 'tool_result', 'block type should be tool_result')

  -- Verify buffer content
  local lines = vim.api.nvim_buf_get_lines(ui._state.sidebar_buf, 0, -1, false)
  local found_result = false
  for _, line in ipairs(lines) do
    if line:match '> %[result: read_file%] success' then
      found_result = true
      break
    end
  end
  assert_true(found_result, 'should find result header in buffer')
end
...
local function test_show_tool_result_error()
  ...
  ui.complete_tool('error_test', 'error')

  assert_no_error(function()
    ui.show_tool_result('error_test', 'command failed', true)
  end, 'show_tool_result with error should not throw')

  local result_id = 'result_error_test'
  assert_eq(ui._state.content_blocks[result_id].state, 'error', 'result state should be error')

  local lines = vim.api.nvim_buf_get_lines(ui._state.sidebar_buf, 0, -1, false)
  local found_error = false
  for _, line in ipairs(lines) do
    if line:match '> %[result: bash%] error' then
      found_error = true
      break
    end
  end
  assert_true(found_error, 'should find error status in buffer')
end

-- Test: foldexpr handles tool headers
local function test_foldexpr_tool_headers()
  setup()
  ui.show_sidebar()
  ui.append_message('assistant', '')
  ui.show_tool_use('fold_test', 'read_file', { path = 'test.lua' })

  vim.api.nvim_set_current_buf(ui._state.sidebar_buf)

  -- Find the tool header line
  local lines = vim.api.nvim_buf_get_lines(ui._state.sidebar_buf, 0, -1, false)
  for i, line in ipairs(lines) do
    if line:match '^> %[tool:' then
      vim.v.lnum = i
      assert_eq(ui.foldexpr(), '>2', 'tool header should start level 2 fold')
      break
    end
  end
end
...
local function test_clear_resets_content_blocks()
  ...
  assert_eq(ui._state.current_tool_id, nil, 'current_tool_id should be nil after clear')
end
...
function M.run()
  local tests = {
    ...
    { name = 'tool: update_tool_input accumulates', fn = test_update_tool_input },
    { name = 'tool: complete_tool finalizes', fn = test_complete_tool },
